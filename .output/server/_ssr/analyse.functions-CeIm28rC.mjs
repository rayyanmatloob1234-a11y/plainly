...
import { T as TSS_SERVER_FUNCTION, a as createServerFn } from "./server-BZt1QFpg.mjs";
...
const MAX_DOCUMENT_CHARS = 18e3;
const GEMINI_URL = (model) => `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`;
const FlagSchema = objectType({
  severity: enumType(["red", "amber", "green"]),
  title: stringType(),
  explanation: stringType(),
  action: stringType().nullable().optional()
});
const HighlightSchema = objectType({
  label: stringType(),
  quote: stringType(),
  explanation: stringType(),
  severity: enumType(["red", "amber", "green"]).optional()
});
const LabValueSchema = objectType({
  name: stringType(),
  value: stringType(),
  unit: stringType().nullable().optional(),
  referenceRange: stringType().nullable().optional(),
  status: enumType(["normal", "low", "high", "ask_doctor"])
});
const GeminiResultSchema = objectType({
  docType: stringType(),
  company: stringType().nullable().optional(),
  country: stringType().nullable().optional(),
  language: stringType().nullable().optional(),
  riskLevel: enumType(["normal", "attention", "urgent", "incorrect"]).optional(),
  summary: stringType(),
  highlights: arrayType(HighlightSchema).optional().default([]),
  flags: arrayType(FlagSchema),
  actions: arrayType(stringType()),
  labValues: arrayType(LabValueSchema).optional().default([])
});
...
function clampDocumentText(text, maxChars = MAX_DOCUMENT_CHARS) {
...
async function callGemini(apiKey, model, parts, jsonOutput, systemPrompt) {
  const body = {
    systemInstruction: {
      role: "system",
      parts: [{
        text: PLAINLY_SYSTEM_PROMPT
      }]
    },
    contents: [{
      role: "user",
...
const analyseDocument_createServerFn_handler = createServerRpc({
  id: "a8745f03bbf65999e2be8d58fc8c995faccd0c4bf6c5cc888cc9939efe268608",
  name: "analyseDocument",
  filename: "src/lib/analyse.functions.ts"
}, (opts) => analyseDocument.__executeServer(opts));
const analyseDocument = createServerFn({
  method: "POST"
}).inputValidator((input) => AnalyseInput.parse(input)).handler(analyseDocument_createServerFn_handler, async ({
  data
}) => {
  ...
  const parts = [];
  parts.push({
    text: `Explain this document in ${data.language}. Return ONLY the JSON object described in the system prompt.

` + (documentText ? `${data.fileName ? `Document name: ${data.fileName}

` : ""}Document text:

${documentText}` : `The user uploaded a document${data.fileName ? ` named "${data.fileName}"` : ""}. Read it and explain it.`)
  });
...
const chatFollowUp_createServerFn_handler = createServerRpc({
  id: "d2ff833751e4fa8be9863691e89bab18fdc3a35b445146213ef04c62fa0cf068",
  name: "chatFollowUp",
  filename: "src/lib/analyse.functions.ts"
}, (opts) => chatFollowUp.__executeServer(opts));
const chatFollowUp = createServerFn({
  method: "POST"
}).inputValidator((input) => ChatInput.parse(input)).handler(chatFollowUp_createServerFn_handler, async ({
  data
}) => {
  const key = process.env.GEMINI_API_KEY;
  if (!key) throw new Error("Missing GEMINI_API_KEY");
  ...
  const contents = [...data.history.map((m) => ({
...
const compareDocuments_createServerFn_handler = createServerRpc({
  id: "7ae4ad10439d6124e7dce91144dcc123f3581053eb720dcd8d8d3b19f6263fe8",
  name: "compareDocuments",
  filename: "src/lib/analyse.functions.ts"
}, (opts) => compareDocuments.__executeServer(opts));
const compareDocuments = createServerFn({
  method: "POST"
}).inputValidator((input) => CompareInput.parse(input)).handler(compareDocuments_createServerFn_handler, async ({
  data
}) => {
  const key = process.env.GEMINI_API_KEY;
  if (!key) throw new Error("Missing GEMINI_API_KEY");
...
OLDER DOCUMENT:
${clampDocumentText(data.doc1Text, 9e3)}

---

NEWER DOCUMENT:
${clampDocumentText(data.doc2Text, 9e3)}`
  }];
...
export {
  analyseDocument_createServerFn_handler,
  chatFollowUp_createServerFn_handler,
  compareDocuments_createServerFn_handler
};
